...
public class Book2 : MonoBehaviour {
    ...
    public Vector3 transformPoint(Vector3 touchScreenPos)
    {
        if (canvas.renderMode == RenderMode.ScreenSpaceCamera)
        {
            Vector3 touchWorldPos = canvas.worldCamera.ScreenToWorldPoint(new Vector3(touchScreenPos.x, touchScreenPos.y, canvas.planeDistance));
            Vector2 localPos = BookPanel.InverseTransformPoint(touchWorldPos);
            return localPos;
        }
        ...
        else
        {
            ...
            Vector2 localPos = BookPanel.InverseTransformPoint(touchScreenPos);
            return localPos;
        }
    }
    void Update()
    ...
    public void UpdateBook()
    {
        ...
        {
            Debug.Log("Toque detectado. Posición: " + Touchscreen.current.primaryTouch.position.ReadValue());
            Vector2 touchPos = Touchscreen.current.primaryTouch.position.ReadValue();
            f = Vector3.Lerp(f, transformPoint(touchPos), Time.deltaTime * 10);
    ...
    public void OnTouchDragRightPage()
    {
        if (interactable)
        {
            Vector2 touchPos = Touchscreen.current.primaryTouch.position.ReadValue();
            DragRightPageToPoint(transformPoint(touchPos));
        }
    }

    public void OnTouchDragLeftPage()
    {
        if (interactable)
        {
            Vector2 touchPos = Touchscreen.current.primaryTouch.position.ReadValue();
